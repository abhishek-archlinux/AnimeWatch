...
class BrowserPage(QWebPage):  
	...
	def _loadProgress(self):
		...
		if self.cnt == 0 and os.path.exists(cookie_file) and ('kisscartoon' in self.url or 'kissasian' in self.url) and self.quality:
			frame = self.mainFrame()
			html = frame.toHtml()
		...
		for cookie in  listCookies:
			...
			for i in j:
				i = re.sub("'",'',i)
				if 'kissanime.to' in i or 'kissasian.com' in i or 'kisscartoon.me' in i or 'masterani.me' in i or 'animeget.io' in i:
