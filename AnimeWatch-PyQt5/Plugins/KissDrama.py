        ...
        if get_cookie or get_link:
            if get_cookie:
                ...
                hls_exec = [
                    ...
                    '--cookie-end-pt=_gat', '--cookie-domain-name=kissasian.ch', 
...
class KissDrama():
    def __init__(self, tmp):
        ...
        """
        self.js_file = os.path.join(tmp, 'kd.js')
    ...
    def search(self, name):
        ...
        if name != '':
            url = 'http://kissasian.ch/Search/Drama/?keyword=' + name
    ...
    def getEpnList(self, name, opt, depth_list, extra_info, siteName, category):
        if extra_info == '-1':
            ...
            url = 'http://kissasian.ch/Drama/' + name
            print(url)
            content = self.ccurlN(url)
            epl = re.findall('/Drama/' + name +'/' +'[^"]*["?"]id[^"]*', content)
            try:
                img = re.findall('http://kissasian.ch/Uploads/Etc/[^"]*.jpg', content)
                print(img)
    ...
    def getFinalUrl(self, name, epn, mirror, quality):
        if '--' in name and 'id=' in name:
            name = name.split('--')[0]
        url = 'http://kissasian.ch/Drama/' + name + '/' + epn
        print(url)
    ...
    def getCompleteList(self, opt, genre_num):
        ...
        self.pg_num = 1
        if opt == 'Genre':
            url = 'http://kissasian.ch/DramaList/'
            content = self.ccurlN(url)
            ...
            url = 'http://kissasian.ch/DramaList/' + opt
            pgn = 1
            content = self.ccurlN(url)
            m = re.findall('/Drama/[^"]*', content)
            m = list(set(m))
            ...
            url = 'http://kissasian.ch/Genre/' + opt
            pgn = 1
            content = self.ccurlN(url)
            m = re.findall('/Drama/[^"]*', content)
            m = list(set(m))
    ...
    def getNextPage(self, opt, pgn, genre_num, name, prev_pg=None):
        ...
        if opt and self.pg_num >= 1:
            ...
            if (opt == 'MostPopular' or opt == 'Newest' or opt == 'LatestUpdate'):
                url = 'http://kissasian.ch/DramaList/' + opt + '?page=' + pgnum
            else:
                url = 'http://kissasian.ch/Genre/' + opt + '?page=' + pgnum
                #print(url
            content = self.ccurlN(url)
            m = re.findall('/Drama/[^"]*', content)
            m = list(set(m))
