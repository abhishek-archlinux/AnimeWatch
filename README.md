...
1. Arch users first need to install 'python-pytaglib' from AUR. Then they can directly go to Release section, and download appropriate pkg.tar.xz package and install it using 'sudo pacman -U pkg_name'. Alternatively they can use PKGBUILD located in Arch related directory and can build package by themselves using 'makepkg -s' command, and then install package using 'pacman'. AnimeWatch is now available in AUR (thanks to Arch linux forum user sesese9) as https://aur.archlinux.org/packages/animewatch-pyqt4 [animewatch-pyqt4] and https://aur.archlinux.org/packages/animewatch-pyqt5 [animewatch-pyqt5]. 
