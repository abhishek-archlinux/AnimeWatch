	...
	def Metadata(self):
		...
		if epnArrList and (site == "Music" or site == "PlayLists"):
			try:
				...
				d = {'xesam:title': str(t),
			    'xesam:artist': (str(art)), 'xesam:album': str("None")}
			except:
				d={'xesam:title':'AnimeWatch','xesam:artist': ["AnimeWatch"]}
...
class ExtendedQLabelEpn(QtGui.QLabel):
	...
	def mouseReleaseEvent(self, ev):
		...
		if ev.button() == QtCore.Qt.LeftButton:
			...
			if '#' in epn_name_in_list:
				newTitle = epn_name_in_list
			else:
				newTitle = '#'+epn_name_in_list	
			sumry = "<html><h1>"+epn_name_in_list+"</h1></html>"
	...
	def contextMenuEvent(self, event):
		...
		if action == queue_item:
			if site == "Music" or site == "Video" or site == "Local" or site == "PlayLists" or site == "None":
				...
				self.seek_timer.connect(self.seek_timer,QtCore.SIGNAL("timeout()"),self.seek_mplayer)
				...
				ui.labelFrame2.setText(epn_name_in_list)
		elif action == watch1:
			#if site=="Local":
				if mpvplayer:
					if mpvplayer.pid()>0:
			...
			print ("creating")
			item, ok = QtGui.QInputDialog.getText(MainWindow, 'Input Dialog', 'Enter Playlist Name')
			if ok and item:
			...
			if site == "Local" or finalUrlFound == True or site=="None" or site =="PlayLists" or site == "Video" or site == "Music":
				...
				if interval == 100:
					...
					subprocess.call(["mpv","--ytdl=no","--quiet","--no-audio","--vo=image:outdir=/tmp/AnimeWatch","--start="+str(interval)+"%","--frames=1",path_final_Url])
					if os.path.exists('/tmp/AnimeWatch/00000001.jpg'):
						shutil.copy('/tmp/AnimeWatch/00000001.jpg',picn)
						os.remove('/tmp/AnimeWatch/00000001.jpg')
					img = QtGui.QPixmap(picn, "1")			
...
class QLineCustomEpn(QtGui.QLineEdit):
	def __init__(self, parent):
...
class QtGuiQWidgetScroll1(QtGui.QScrollArea):
	def __init__(self, parent):
...
class tab6(QtGui.QWidget):
	...
	def resizeEvent(self,event):
		...
		if ui.tab_6.width() > 500 and tab_6_player == "False" and iconv_r != 1:
				#browse_cnt = 0
				if tab_6_size_indicator:
					tab_6_size_indicator.pop()
				tab_6_size_indicator.append(ui.tab_6.width())
	...
	def keyPressEvent(self, event):
		...
		if event.modifiers() == QtCore.Qt.ControlModifier and event.key() == QtCore.Qt.Key_Right:
			...
			if Player == "mpv":
				...
				print (t)
				mpvplayer.write(t.encode('utf-8'))
				#mpvplayer.write('\n'+'osd_show_text '+(sz)+' 4000'+'\n')
		elif event.key() == QtCore.Qt.Key_E:
...
class QDockNew(QtGui.QDockWidget):
	def __init__(self, parent):
		global cycle_pause,total_seek
...
class Ui_MainWindow(object):
	def setupUi(self, MainWindow):
		...
		self.superTab = QtGui.QWidget(MainWindow)
		self.superTab.setObjectName(_fromUtf8("superTab"))
		self.superGridLayout = QtGui.QGridLayout(MainWindow)
		self.superGridLayout.setObjectName(_fromUtf8("superGridLayout"))
		self.gridLayout = QtGui.QGridLayout(self.superTab)
		...
		self.text = QtGui.QTextBrowser(MainWindow)
		...
		self.btnEpnList = QtGui.QComboBox(MainWindow)
		...
		self.hide_btn_list1_menu = QtGui.QMenu()
		...
		self.filter_btn = QtGui.QPushButton(self.frame)
		...
		self.goto_epn = QtGui.QFrame(MainWindow)
		self.goto_epn.setFrameShape(QtGui.QFrame.NoFrame)
		self.goto_epn.setFrameShadow(QtGui.QFrame.Raised)
		self.goto_epn.setObjectName(_fromUtf8("goto_epn"))
		self.horizontalLayout_goto_epn = QtGui.QHBoxLayout(self.goto_epn)
		...
		self.frame1 = QtGui.QFrame(MainWindow)
		...
		self.horizontalLayout_31 = QtGui.QVBoxLayout(self.frame1)
		...
		self.frame2 = QtGui.QFrame(MainWindow)
		...
		self.horizontalLayout_101= QtGui.QHBoxLayout(self.frame2)
		...
		self.progressEpn = QtGui.QProgressBar(self.frame1)
		...
		self.player_opt = QtGui.QFrame(self.frame1)
		self.player_opt.setFrameShape(QtGui.QFrame.NoFrame)
		self.player_opt.setFrameShadow(QtGui.QFrame.Raised)
		self.player_opt.setObjectName(_fromUtf8("player_opt"))
		self.horizontalLayout_player_opt = QtGui.QHBoxLayout(self.player_opt)
		...
		self.player_opt_toolbar= QtGui.QPushButton(self.player_opt)
		...
		self.sd_hd = QtGui.QPushButton(self.player_opt)
		...
		self.audio_track = QtGui.QPushButton(self.player_opt)
		...
		self.subtitle_track = QtGui.QPushButton(self.player_opt)
		...
		self.queue_manage = QtGui.QPushButton(self.goto_epn)
		...
		self.mirror_change = QtGui.QPushButton(self.goto_epn)
		...
		self.goto_epn_filter = QtGui.QPushButton(self.goto_epn)
		...
		self.player_playlist1 = QtGui.QPushButton(self.goto_epn)
		...
		self.VerticalLayoutLabel_Dock3 = QtGui.QVBoxLayout(self.dockWidgetContents_3)
		...
		self.line = QtGui.QLineEdit(self.dockWidgetContents_3)
		...
		#self.web.hide()
		self.verticalLayout_50 = QtGui.QHBoxLayout()
		self.verticalLayout_50.setObjectName(_fromUtf8("verticalLayout_50"))
		self.horizontalLayout_5.addLayout(self.verticalLayout_50)
		self.btnWebClose = QtGui.QPushButton(self.tab_2)
		...
		self.btnWebReviews = QtGui.QComboBox(self.tab_2)
		...
		self.btn2 = QtGui.QComboBox(self.dockWidgetContents_3)
		#self.btn2.setGeometry(QtCore.QRect(20, 315, 91, 31))
		self.btn2.setObjectName(_fromUtf8("btn2"))
		self.btn3 = QtGui.QPushButton(self.dockWidgetContents_3)
		...
		self.page_number = QtGui.QLineEdit(self.frame)
		...
		#self.label.setStyleSheet(_fromUtf8("background-image: url(:/home/abhishek/soft/python/1.jpg);"))
		
		self.horizontalLayout10 = QtGui.QVBoxLayout(self.tab_6)
		...
		self.gridLayout1 = QtGui.QGridLayout(self.scrollAreaWidgetContents)
		...
		self.gridLayout2 = QtGui.QGridLayout(self.scrollAreaWidgetContents1)
		...
		self.btn10 = QtGui.QComboBox(self.scrollAreaWidgetContents)
		...
		self.comboBox20 = QtGui.QComboBox(self.scrollAreaWidgetContents)
		...
		self.label_search = QtGui.QLineEdit(self.scrollAreaWidgetContents)
		self.label_search.setObjectName(_fromUtf8("label_search"))
		self.label_search.setMaximumWidth(100)
		
		self.btn201 = QtGui.QPushButton(self.scrollAreaWidgetContents)
		...
		self.chk = QtGui.QComboBox(self.dockWidget_3) 
		self.chk.setObjectName(_fromUtf8("chk"))
		...
		self.comboView = QtGui.QComboBox(self.dockWidget_3) 
		...
		self.btn4 = QtGui.QPushButton(self.dockWidgetContents_3)
		...
		self.frame_timer.connect(self.frame_timer,QtCore.SIGNAL("timeout()"),self.frame_options)
		...
		self.comboView.addItem(_fromUtf8(""))
		self.comboView.addItem(_fromUtf8(""))
		QtGui.QShortcut(QtGui.QKeySequence("Shift+F"), MainWindow, self.fullscreenToggle)
		QtGui.QShortcut(QtGui.QKeySequence("L"), MainWindow, self.setPlayerFocus)
		QtGui.QShortcut(QtGui.QKeySequence("G"), MainWindow, self.dockShowHide)
		...
		QtGui.QShortcut(QtGui.QKeySequence("Ctrl+F"), MainWindow, self.searchAnime)
		QtGui.QShortcut(QtGui.QKeySequence("Ctrl+Z"), MainWindow, self.IconView)
		QtGui.QShortcut(QtGui.QKeySequence("Ctrl+X"), MainWindow, self.showHideBrowser)
		QtGui.QShortcut(QtGui.QKeySequence("ESC"), MainWindow, self.HideEveryThing)
		...
		#QtCore.QObject.connect(self.slider, QtCore.SIGNAL(_fromUtf8("valueChanged(int)")), self.moveSlider)
		...
		#QtCore.QObject.connect(self.btn20, QtCore.SIGNAL(_fromUtf8("clicked()")), self.go_opt_options_btn20)
		QtCore.QObject.connect(self.btn10, QtCore.SIGNAL(_fromUtf8("currentIndexChanged(int)")), self.browse_epn)
		#QtCore.QObject.connect(self.btnEpnList, QtCore.SIGNAL(_fromUtf8("currentIndexChanged(int)")), self.PlayEpn)
		...
		#QtCore.QObject.connect(self.list1, QtCore.SIGNAL(_fromUtf8("itemClicked(QListWidgetItem*)")), self.list_highlight)
		...
		#QtCore.QObject.connect(self.btn9, QtCore.SIGNAL(_fromUtf8("clicked()")), self.launchPlayer)
		#QtCore.QObject.connect(self.label, QtCore.SIGNAL(_fromUtf8("clicked()")), self.showImage)
		#QtCore.QObject.connect(self.go_page, QtCore.SIGNAL(_fromUtf8("returnPressed()")), self.directgo)
		...
		#QtCore.QObject.connect(self.goto_epn, QtCore.SIGNAL(_fromUtf8("textChanged(QString)")), self.filter_epn_list)
		QtCore.QObject.connect(self.goto_epn, QtCore.SIGNAL(_fromUtf8("returnPressed()")), self.directepn)
		#QtCore.QObject.connect(self.btn3, QtCore.SIGNAL(_fromUtf8("clicked()")), self.reviews)
		...
		#QtCore.QObject.connect(self.btn2, QtCore.SIGNAL(_fromUtf8("currentIndexChanged(int)")), self.reviews)
		#self.scrollArea.verticalScrollBar().valueChanged.connect(self.load_more)
		#self.scrollArea1.verticalScrollBar().valueChanged.connect(self.adjustVideoScreen)
		#self.scrollArea1.verticalScrollBar().valueChanged.connect(self.scrollarea1_position)
		#QtCore.QObject.connect(return1, QtCore.SIGNAL('activated()'), self.listfound)
		#QtCore.QObject.connect(return2, QtCore.SIGNAL('activated()'), self.epnfound)
		#self.tabWidget.setCurrentIndex(0)
		#QtCore.QObject.connect(self.forward, QtCore.SIGNAL(_fromUtf8("clicked()")), self.hello)
		#QtCore.QObject.connect(self.chk, QtCore.SIGNAL(_fromUtf8("clicked()")), self.preview)
	...
	def addToLibrary(self):
		...
		self.RemoveLibraryFolder = QtGui.QPushButton(self.LibraryDialog)
		...
		self.LibraryClose = QtGui.QPushButton(self.LibraryDialog)
	...
	def next_page(self,value_str):
			...
			if total_till==0 or value_str=="not_deleted":
				...
				while(i<length):
					...
					if value_str == "deleted":
						#p1="self.label_"+str(i)+" = ExtendedQLabel(self.scrollAreaWidgetContents)"
						p1="self.label_"+str(length+i)+" = QtGui.QLineEdit(self.scrollAreaWidgetContents)"
	...
	def thumbnailEpn(self):
			...
			if total_till_epn==0:
				...
				while(i<length1):
					...
					p1="self.label_epn_"+str(i)+" = QtGui.QLineEdit(self.scrollAreaWidgetContents1)"
					p7 = "l_"+str(i)+" = weakref.ref(self.label_epn_"+str(i)+")"
					#p2="self.label_"+str(i)+".setMaximumSize(QtCore.QSize(300,200))"
					#p2="self.label_"+str(i)+".setMaximumSize(QtCore.QSize("+width+","+height+"))"
					p2="self.label_epn_"+str(i)+".setMinimumWidth("+width+")"
			...
			if site == "Music":
				...
				if (not os.path.isfile(fan_text) or not os.path.isfile(post_text)):
					if not nav:
						...
						#if option != "FindAll":
						if posterManually == 1:
							scode, ok = QtGui.QInputDialog.getText(MainWindow, 'Input Dialog', 'Enter Name Manually \n or prefix "url:" for direct url \n or "g:" for google Search')
							if ok and scode:
							#scode = subprocess.check_output(["zenity","--entry","--text","Enter Anime Name Manually"])
								scode = str(scode)
								nam = re.sub("\n","",scode)
	...
	def nextp(self,val):
		...
		global opt,pgn,genre_num,site,embed,mirrorNo,quality
		...
		try:
			...
			pgn = pgn + 1
			if (opt != "") and (pgn >= 1):
				m = site_var.getNextPage(opt_val,pgn,genre_num)
				self.list1.clear()
				for i in m:
					self.list1.addItem(i)
	...
	def backp(self,val):
		...
		global opt,pgn,genre_num,embed,mirrorNo,quality
		self.list1.verticalScrollBar().setValue(self.list1.verticalScrollBar().minimum())
		if val == "back":
			r = self.list3.currentRow()
		else:
	...
	def rawlist_highlight(self):
		...
		if os.path.exists(file_name) and site!="PlayLists":
			...
			if os.path.isfile(str(picn)):
				...
				picn = thumbnail
				#picn = "/tmp/AnimeWatch/"+name+'.jpg'	
				picn1 = "/home/abhishek/Templates/animebeta/1.png"
	...
	def search(self):
		...
		global pict_arr,name_arr,summary_arr,total_till,browse_cnt,tmp_name,list2_items,bookmark,refererNeeded
		pict_arr[:]=[]
		name_arr[:]=[]
		summary_arr[:]=[]
		#total_till=0
			...
			if self.list1.currentItem():
				...
				if os.path.isfile(str(picn)):
					...
					if not os.path.isfile(thumbnail):
						...
						img.save(str(thumbnail))
					picn = thumbnail	
					picn1 = "/home/abhishek/Templates/animebeta/1.jpg"
					tmp = '"background-image: url('+fanart+')"'
				...
				if self.list1.currentItem():
					...
					if not os.path.exists(music_dir_art_name):
						...
						if not os.path.exists(poster) and srch != "offline":	
							...
							QtCore.QObject.connect(self.threadPool[len(self.threadPool)-1], QtCore.SIGNAL("finished()"),self.finishedM)
							#self.threadEx.start()
		...
		if site == "PlayLists":
			...
			if item:
				...
				if len(arr) > 2:
					...
					refererNeeded = False
				epn_name_in_list = arr[0]
				epn = epn_name_in_list
				self.playlistUpdate()
		...
		if(site != "SubbedAnime" and site!= "DubbedAnime" and site!="PlayLists" and finalUrlFound == False and site !="None" and site!= "Music" and site != "Video" and site!= "Local") :
			...
			if site == "SubbedAnime":
				...
				if opt_movies_indicator:
					r = self.list2.currentRow()
					epn_name_in_list = self.list2.currentItem().text()
					#cmd = site +"()"
					#site_var=eval(cmd)
					module = imp.load_source(site,home+'/src/Plugins/'+site+'.py')
					site_var = getattr(module,site)()
	...
	def localGetInList(self):
		...
		if site != "PlayLists":
			if '	' in epnArrList[row]:
				epn = epnArrList[row].split('	')[1]
				epn_name_in_list = (epnArrList[row]).split('	')[0]
			else:
				epn = self.list2.currentItem().text()
				epn_name_in_list = str(epn)
			if not epn:
				return 0
			epn = epn.replace('#','')
		else:
			...
			if Player == "mplayer":
				...
				if mpvplayer.pid()>0:
					try:
						...
						mpvplayer.write(t1.encode('utf-8'))
